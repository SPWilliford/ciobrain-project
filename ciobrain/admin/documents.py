"""
ciobrain/admin/documents.py

This module defines classes required for document management. 
...
class StorageHandler:
    ...
    def get_directory_contents(self) -> dict[str, list[str]]:
        ...
        directories = {
            'original_uploads': os.listdir(current_app.config['ORIGINAL_UPLOADS_FOLDER']),
            'in_progress': os.listdir(current_app.config['IN_PROGRESS_FOLDER']),
            'final_staging': os.listdir(current_app.config['FINAL_STAGING_FOLDER']),
        }
        return directories
    ...
    def _save_file(self, file) -> str:
        ...
        uploads_directory = current_app.config['ORIGINAL_UPLOADS_FOLDER']
